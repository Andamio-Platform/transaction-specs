...
func main() {
	...
	for _, hash := range txHashes {
		...
		if useraccesstoken.Mint(tx, accessTokenPolicy) {
			fmt.Println("Handler: UserAccessToken → MINT")
			classified = true
		}
		...
		if admincourse.UpdateTeachers(tx) {
			fmt.Println("Handler: AdminCourse → UPDATE_TEACHERS")
			classified = true
		}
		...
		if teachercourse.ManageModules(tx, moduleScriptsV2PolicyId) {
			fmt.Println("Handler: TeacherCourse → MANAGE_MODULES")
			classified = true
		}
		...
		if studentcourse.SubmitAssignment(tx) {
			fmt.Println("Handler: StudentCourse → SUBMIT_ASSIGNMENT")
			classified = true
		}

		if studentcourse.UpdateAssignment(tx) {
			fmt.Println("Handler: StudentCourse → UPDATE_ASSIGNMENT")
			classified = true
		}

		if studentcourse.ClaimCredential(tx, courseStatePolicyIds) {
			fmt.Println("Handler: StudentCourse → CLAIM_CREDENTIAL")
			classified = true
		}
